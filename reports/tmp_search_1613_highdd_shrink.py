...
OUT_CSV = "reports/search_1613_highdd_shrink_candidates.csv"
...
def load_model_and_ranked(alphas):
    ...
    ranked_map = {}
    for alpha in sorted(set(int(a) for a in alphas)):
        scored = base.copy()
    ...
    print(f"prepared_alphas={sorted(ranked_map.keys())}")
...
def main() -> None:
    ...
    # high-return but higher-drawdown families; shrink alloc/threshold to fit DD<=16.13
    seeds = coarse[(coarse["ret_pct"] >= 1400.0) & (coarse["dd_pct"] <= 30.0)].copy()
    seeds = seeds.sort_values(["ret_pct", "dd_pct"], ascending=[False, True])
    seeds = seeds.drop_duplicates(subset=key_cols).head(20).copy()
    print(f"seed_configs={len(seeds)}")

    rules, trade_dates, ranked_map = load_model_and_ranked(seeds["alpha"].tolist())

    th_grid = [round(x, 4) for x in np.arange(0.50, 0.901, 0.01)]
    alloc_grid = [round(x, 4) for x in np.arange(0.60, 1.0001, 0.005)]
    print(f"grid thresholds={len(th_grid)} allocs={len(alloc_grid)}")

    rows = []
    checked = 0
    best = None

    for i, row in seeds.reset_index(drop=True).iterrows():
        cfg = {
            "alpha": int(row["alpha"]),
            "sell_rule": str(row["sell_rule"]),
            "top_k": int(row["top_k"]),
            "gap_min": _opt_float(row["gap_min"]),
            "gap_max": _opt_float(row["gap_max"]),
            "max_board": _opt_int(row["max_board"]),
            "max_broken_rate": _opt_float(row["max_broken_rate"]),
            "min_red_rate": _opt_float(row["min_red_rate"]),
            "max_limit_down": _opt_int(row["max_limit_down"]),
            "max_pullback": _opt_float(row["max_pullback"]),
        }
        ranked = ranked_map[cfg["alpha"]]
        for th in th_grid:
        ...
        print(f"[cfg-done] {i+1}/{len(seeds)} checked={checked}")

    if best is not None:
        b = best[2]
        ranked = ranked_map[int(b["alpha"])]
        th2 = [round(x, 4) for x in np.arange(max(0.5, b["threshold"] - 0.03), min(0.99, b["threshold"] + 0.03) + 1e-12, 0.001)]
        alloc2 = [round(x, 4) for x in np.arange(max(0.4, b["alloc"] - 0.05), min(1.0, b["alloc"] + 0.05) + 1e-12, 0.001)]
        print(f"ultra thresholds={len(th2)} allocs={len(alloc2)}")
        for th in th2:
            for alloc in alloc2:
                m = mb._simulate_fast(
                    trade_dates=trade_dates,

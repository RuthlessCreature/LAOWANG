...
OUT_CSV = "reports/search_npz_scorebase_v8.csv"
OUT_SUMMARY = "reports/search_npz_scorebase_v8.txt"
...
HP_CANDIDATES = [
    ...
    {"hidden": 88, "epochs": 500, "lr": 0.0210, "weight_decay": 0.00020, "target_gt": 0.0100, "seed": 439, "tag": "seed_near"},
    {"hidden": 88, "epochs": 500, "lr": 0.0210, "weight_decay": 0.00020, "target_gt": 0.0100, "seed": 441, "tag": "seed_near"},
]

SCORE_BASE_STARTS = ["2024-01-01", "2024-06-01", "2024-09-01", "2024-12-01", "2025-01-01", "2025-03-01"]
...
STAGE2_ALPHA = [14, 15, 16, 17]
STAGE2_THRESHOLD = [0.58, 0.60, 0.62, 0.64, 0.66]
STAGE2_ALLOC = [0.9980, 0.9990, 0.9998]
...
def main() -> None:
    ...
    cache: Dict[Tuple[float, ...], pd.Series] = {}
    ...
    for i, hp in enumerate(HP_CANDIDATES, 1):
        ...
        for s_idx, score_base_start in enumerate(SCORE_BASE_STARTS, 1):
            ...
            for cfg in STAGE1_CFGS:
                if ranked_a15 is None:
                    score = score_from_ref(base_prob, ref, int(cfg["alpha"]))
                    scored = base.copy()
                    scored["raw_prob"] = base_prob
                    scored["score"] = score
                    ranked_a15 = mb._build_ranked_by_date(scored)

                ret_pct, dd_pct, trades = simulate(data, ranked_a15, cfg)
    ...
    stage2_cfgs = [
        ...
        for alpha, th, alloc, (gmin, gmax) in itertools.product(
            STAGE2_ALPHA,
            STAGE2_THRESHOLD,
            STAGE2_ALLOC,
            STAGE2_GAPS,
    ...
    for idx, (_, __, hp, score_base_start) in enumerate(stage2_parents, 1):
        prob_full = cache[hp_key(hp)]
        base_prob = prob_full.reindex(base.index).to_numpy(dtype=float)
        ref = prob_full.reindex(full_pool[full_pool["date"] >= score_base_start].index).to_numpy(dtype=float)
        ref = ref[np.isfinite(ref)]
        if ref.size < 100:
        ...
        for cfg in stage2_cfgs:
            ...
            if dd_pct <= DD_LIMIT and ret_pct > best_ret:
                ...
                best_ret = ret_pct
                print(
                    f"[best<=16.13 stage2] parent={idx}/{len(stage2_parents)} "
                    f"ret={ret_pct:.6f}% dd={dd_pct:.6f}% hp={hp} score_base_start={score_base_start} cfg={cfg}"
                )

        print(f"stage2 progress {idx}/{len(stage2_parents)}")

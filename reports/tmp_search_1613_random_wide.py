...
OUT_CSV = "reports/search_1613_random_wide_candidates.csv"


def objective(ret_pct: float, dd_pct: float) -> float:
    return ret_pct - 55.0 * max(0.0, dd_pct - DD_LIMIT)
...
def sample_cfg(rng):
    ...
    sell_rule = rng.choice(["t2_close", "strong_hold_t3", "loss_rebound_t3"])
    top_k = rng.choice([2, 3, 4, 5])

    gap_min = round(rng.uniform(-0.012, 0.000), 4)
    gap_max = round(rng.uniform(0.020, 0.040), 4)
    if gap_max < gap_min + 0.015:
        gap_max = round(gap_min + 0.015, 4)

    max_board = rng.choice([None, 1, 2, 3, 4])

    if rng.random() < 0.08:
        max_broken_rate = None
        min_red_rate = None
        max_limit_down = None
        max_pullback = None
    else:
        max_broken_rate = round(rng.uniform(0.34, 0.50), 3)
        min_red_rate = round(rng.uniform(0.16, 0.33), 3)
        if min_red_rate > max_broken_rate - 0.006:
            min_red_rate = round(max(0.10, max_broken_rate - rng.uniform(0.010, 0.10)), 3)
        max_limit_down = int(rng.randint(8, 30))
        max_pullback = round(rng.uniform(0.06, 0.13), 3)

    threshold = round(rng.uniform(0.42, 0.66), 4)
    alloc = round(rng.uniform(0.88, 1.00), 4)
...
def mutate(cfg, rng):
    ...
    c["sell_rule"] = rng.choice([c["sell_rule"], "t2_close", "strong_hold_t3", "loss_rebound_t3"])
    c["top_k"] = int(clip(c["top_k"] + rng.choice([-1, 0, 1]), 1, 5))

    c["gap_min"] = round(clip(float(c["gap_min"]) + rng.uniform(-0.003, 0.003), -0.015, 0.003), 4)
    c["gap_max"] = round(clip(float(c["gap_max"]) + rng.uniform(-0.004, 0.004), 0.015, 0.045), 4)
    if c["gap_max"] < c["gap_min"] + 0.012:
        c["gap_max"] = round(c["gap_min"] + 0.012, 4)

    if c["max_board"] is None:
        if rng.random() < 0.2:
            c["max_board"] = rng.choice([1, 2, 3, 4])
    else:
        if rng.random() < 0.15:
            c["max_board"] = None
        else:
    ...
    if c["max_broken_rate"] is None:
        if rng.random() < 0.5:
            c["max_broken_rate"] = round(rng.uniform(0.34, 0.50), 3)
            c["min_red_rate"] = round(rng.uniform(0.16, 0.33), 3)
            if c["min_red_rate"] > c["max_broken_rate"] - 0.006:
                c["min_red_rate"] = round(max(0.10, c["max_broken_rate"] - 0.03), 3)
            c["max_limit_down"] = int(rng.randint(8, 30))
            c["max_pullback"] = round(rng.uniform(0.06, 0.13), 3)
    else:
        c["max_broken_rate"] = round(clip(float(c["max_broken_rate"]) + rng.uniform(-0.015, 0.015), 0.32, 0.55), 3)
        c["min_red_rate"] = round(clip(float(c["min_red_rate"]) + rng.uniform(-0.02, 0.02), 0.10, 0.38), 3)
        if c["min_red_rate"] > c["max_broken_rate"] - 0.004:
            c["min_red_rate"] = round(max(0.10, c["max_broken_rate"] - 0.01), 3)
        c["max_limit_down"] = int(clip(int(c["max_limit_down"]) + rng.choice([-2, -1, 0, 1, 2]), 4, 40))
        c["max_pullback"] = round(clip(float(c["max_pullback"]) + rng.uniform(-0.010, 0.010), 0.04, 0.16), 3)

    c["threshold"] = round(clip(float(c["threshold"]) + rng.uniform(-0.03, 0.03), 0.35, 0.75), 4)
    c["alloc"] = round(clip(float(c["alloc"]) + rng.uniform(-0.03, 0.03), 0.70, 1.00), 4)
...
def main():
    ...
    focus = sorted(pool, key=lambda r: (r["obj"], r["ret_pct"]), reverse=True)[:250]

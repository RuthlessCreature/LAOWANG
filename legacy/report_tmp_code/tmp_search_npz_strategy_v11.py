...
OUT_CSV = "reports/search_npz_strategy_v11.csv"
OUT_SUMMARY = "reports/search_npz_strategy_v11.txt"
...
TOP_KS = [2, 3, 4]
ALPHAS = [14, 15, 16, 17, 18]
THRESHOLDS = [0.62, 0.64, 0.66, 0.68, 0.70]
ALLOCS = [0.9990, 0.9998, 1.0000]
GAPS = [(-0.0010, 0.0255), (-0.0005, 0.0255), (-0.0005, 0.0265)]
MAX_BROKEN = [0.406, 0.412]
MIN_RED = [0.275, 0.287]
MAX_LIMIT_DOWN = [10, 12]
MAX_PULLBACK = [0.060, 0.065]
...
def sample_stage2_cfg(rng: random.Random) -> Dict[str, float]:
    cfg = {
        ...
        "gap_min": 0.0,
        "gap_max": 0.0,
        "max_broken_rate": rng.choice(MAX_BROKEN),
        "min_red_rate": rng.choice(MIN_RED),
        "max_limit_down": rng.choice(MAX_LIMIT_DOWN),
        "max_pullback": rng.choice(MAX_PULLBACK),
    }
    cfg["gap_min"], cfg["gap_max"] = rng.choice(GAPS)
...
def main() -> None:
    rng = random.Random(202603040030)
    data = prepare_data()
    ...
    for i, hp in enumerate(HP_CANDIDATES, 1):
        ...
        for s_idx, score_base_start in enumerate(SCORE_BASE_STARTS, 1):
            ...
            for cfg in STAGE1_CFGS:
                ...
                ret_pct, dd_pct, trades = simulate(data, ranked_map[alpha], cfg)
                ...
                if dd_pct <= DD_LIMIT and ret_pct > best_ret:
                    ...
                    best_ret = ret_pct
                    print(
                        f"[best<=16.13 stage1] model={i}/{len(HP_CANDIDATES)} scorebase={s_idx}/{len(SCORE_BASE_STARTS)} "
    ...
    for idx, (_, __, hp, score_base_start) in enumerate(parents, 1):
        ...
        seen = set()
        draws = 0
        while draws < samples_per_parent:
            cfg = sample_stage2_cfg(rng)
            key = (
                cfg["sell_rule"],
                cfg["top_k"],
                cfg["alpha"],
            ...
            if dd_pct <= DD_LIMIT and ret_pct > best_ret:
                ...
                best_ret = ret_pct
                print(
                    f"[best<=16.13 stage2] parent={idx}/{len(parents)} checked={draws} "

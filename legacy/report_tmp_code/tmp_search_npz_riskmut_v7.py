...
OUT_CSV = "reports/search_npz_riskmut_v7.csv"
OUT_SUMMARY = "reports/search_npz_riskmut_v7.txt"
...
def build_candidates(rng: random.Random) -> List[Dict[str, float]]:
    cands: List[Dict[str, float]] = []

    base_block = [
        {"hidden": 88, "epochs": 500, "lr": 0.0210, "weight_decay": 0.00020, "target_gt": 0.0100, "seed": 440, "tag": "incumbent"},
        {"hidden": 88, "epochs": 520, "lr": 0.0205, "weight_decay": 0.00020, "target_gt": 0.0100, "seed": 440, "tag": "tie_hp"},
        {"hidden": 88, "epochs": 500, "lr": 0.0210, "weight_decay": 0.00016, "target_gt": 0.0100, "seed": 440, "tag": "tie_hp"},
        {"hidden": 88, "epochs": 500, "lr": 0.0215, "weight_decay": 0.00022, "target_gt": 0.0100, "seed": 440, "tag": "tie_hp"},
    ]
    cands.extend(base_block)

    for seed in range(434, 447):
        cands.append(
            {
                "hidden": 88,
                "epochs": 500,
    ...
    for seed in [438, 439, 440, 441, 442]:
        cands.append(
            {
                "hidden": 88,
                "epochs": 520,
        ...
        cands.append(
            {
                ...
                "epochs": rng.choice([460, 500, 520, 560]),
                "lr": round(rng.uniform(0.0190, 0.0230), 4),
                "weight_decay": round(rng.uniform(0.00014, 0.00028), 6),
                "target_gt": rng.choice([0.0090, 0.0100, 0.0110, 0.0120]),
                "seed": rng.randint(430, 450),
                "tag": "random_local",
            }
        )
    ...
    out.sort(key=lambda x: (x["tag"] != "incumbent", x["tag"].startswith("random"), x["seed"]))
    return out[:46]


def sample_stage2_cfg(anchor: Dict[str, float], rng: random.Random) -> Dict[str, float]:
    return {
        "alpha": rng.choice([14, 15, 16]),
        "threshold": round(clip(anchor["threshold"] + rng.uniform(-0.04, 0.08), 0.55, 0.68), 4),
        "alloc": round(clip(anchor["alloc"] + rng.uniform(-0.003, 0.001), 0.9950, 1.0000), 4),
        "gap_min": round(clip(anchor["gap_min"] + rng.uniform(-0.0010, 0.0005), -0.0020, 0.0005), 4),
        "gap_max": round(clip(anchor["gap_max"] + rng.uniform(-0.0010, 0.0015), 0.0240, 0.0280), 4),
        "max_broken_rate": round(clip(anchor["max_broken_rate"] + rng.uniform(-0.012, 0.012), 0.396, 0.426), 3),
        "min_red_rate": round(clip(anchor["min_red_rate"] + rng.uniform(-0.040, 0.020), 0.240, 0.305), 3),
        "max_limit_down": int(clip(anchor["max_limit_down"] + rng.choice([-2, -1, 0, 1, 2]), 8, 12)),
        "max_pullback": round(clip(anchor["max_pullback"] + rng.uniform(-0.010, 0.010), 0.055, 0.075), 3),
    }
...
def main() -> None:
    rng = random.Random(202603032243)
    data = prepare_data()
    base: pd.DataFrame = data["base"]
    ...
    for i, hp in enumerate(candidates, 1):
        ...
        for cfg in STAGE1_CFGS:
            full_cfg = dict(cfg)
            full_cfg.update(
                max_broken_rate=RISK_ANCHOR["max_broken_rate"],
                min_red_rate=RISK_ANCHOR["min_red_rate"],
                max_limit_down=RISK_ANCHOR["max_limit_down"],
                max_pullback=RISK_ANCHOR["max_pullback"],
            )
            ranked = alpha_ranked[int(cfg["alpha"])]
            ret_pct, dd_pct, trades = simulate(data, ranked, full_cfg)
            ...
            if dd_pct <= DD_LIMIT and ret_pct > best_ret:
                ...
                best_ret = ret_pct
                print(
                    f"[best<=16.13 stage1] {i}/{len(candidates)} "
                    f"ret={ret_pct:.6f}% dd={dd_pct:.6f}% hp={hp} cfg={full_cfg}"
                )
    ...
    for i, hp in enumerate(parents, 1):
        ...
        for _ in range(stage2_per_parent):
            cfg = sample_stage2_cfg(RISK_ANCHOR, rng)
            if cfg["gap_max"] < cfg["gap_min"] + 0.012:
                cfg["gap_max"] = round(cfg["gap_min"] + 0.012, 4)
            if cfg["min_red_rate"] > cfg["max_broken_rate"] - 0.004:
                cfg["min_red_rate"] = round(max(0.200, cfg["max_broken_rate"] - 0.006), 3)
            ...
            if dd_pct <= DD_LIMIT and ret_pct > best_ret:
                ...
                best_ret = ret_pct
                print(
                    f"[best<=16.13 stage2] {i}/{len(parents)} "
    ...
    if len(feasible):
        cols = [
            ...
            "alpha", "threshold", "alloc", "gap_min", "gap_max",

...
class AppContext:
    ...
    def fetch_plan(self, trade_date: str) -> Dict[str, Any]:
        """Return T-day signal plan for T+1 execution.

        Reads from strategy_signal_log for the given trade_date.
        Returns ordinary model plans (skip/conditional_buy) and relay watchlist.
        """
        ordinary_query = """
            SELECT
                signal_date,
                ...
                CASE
                    ...
                    THEN 'skip'
                    ELSE 'pending_open_check'
                END AS action_state,
                ROUND(COALESCE(json_extract(action_plan_json, '$.position_pct'), 0), 4) AS planned_position_pct,
                json_extract(action_plan_json, '$.hard_stop_pct') AS hard_stop_pct,
                json_extract(action_plan_json, '$.structure_stop') AS structure_stop,
                json_extract(action_plan_json, '$.max_hold_days') AS max_hold_days,
                json_extract(action_plan_json, '$.skip_reason') AS skip_reason
            FROM strategy_signal_log
            WHERE signal_date = :td
              AND model IN ('laowang', 'stwg', 'ywcx', 'fhkq')
            ORDER BY model, planned_position_pct DESC, score DESC
        """
        relay_query = """
            SELECT
                signal_date,
                ...
                features_json,
                action_plan_json,
                'watch_auction' AS action_state,
                ROUND(COALESCE(json_extract(action_plan_json, '$.position_pct'), 0), 4) AS planned_position_pct,
                json_extract(action_plan_json, '$.drop_reason') AS drop_reason,
                json_extract(action_plan_json, '$.exit') AS exit_protocol
            FROM strategy_signal_log
            WHERE signal_date = :td
              AND model = 'relay'
            ORDER BY score DESC
        ...
        with self.engine.connect() as conn:
            ordinary_rows = [
                dict(row) for row in conn.execute(text(ordinary_query), {"td": trade_date}).mappings().all()
            ]
            relay_rows = [
                dict(row) for row in conn.execute(text(relay_query), {"td": trade_date}).mappings().all()
            ]
        ...
        for row in ordinary_rows:
            row = dict(row)
            skip_reason = parse_json_field(row.get("skip_reason"))
            if isinstance(skip_reason, list):
                row["skip_reason"] = "|".join(str(r) for r in skip_reason if r)
            elif skip_reason:
                row["skip_reason"] = str(skip_reason)
            else:
                row["skip_reason"] = ""
            row["action_state"] = "skip" if row.get("skip_reason") else "pending_open_check"
            row["planned_action"] = "skip" if row.get("skip_reason") else "conditional_buy"
            ordinary_parsed.append(row)

        relay_parsed = []
        for row in relay_rows:
            row = dict(row)
            drop_reason = parse_json_field(row.get("drop_reason"))
            if isinstance(drop_reason, list):
                row["drop_reason"] = "|".join(str(r) for r in drop_reason if r)
            elif drop_reason:
                row["drop_reason"] = str(drop_reason)
            else:
                row["drop_reason"] = ""
            row["action_state"] = "drop_candidate" if row.get("drop_reason") else "watch_auction"
            row["planned_status"] = "drop" if row.get("drop_reason") else "candidate"
            relay_parsed.append(row)

        pending = [r for r in ordinary_parsed if r["action_state"] == "pending_open_check"]
        skipped = [r for r in ordinary_parsed if r["action_state"] == "skip"]

        return {
            "trade_date": trade_date,
            "ordinary_plan": {
                "columns": ["model", "stock_code", "stock_name", "score", "action_state", "planned_action", "planned_position_pct", "hard_stop_pct", "structure_stop", "max_hold_days", "skip_reason"],
                "rows": ordinary_parsed,
                "pending_count": len(pending),
                "skipped_count": len(skipped),
            },
            "relay_watchlist": {
                "columns": ["stock_code", "stock_name", "score", "action_state", "planned_position_pct", "drop_reason"],
                "rows": relay_parsed,
                "watch_count": sum(1 for r in relay_parsed if r["action_state"] == "watch_auction"),
                "drop_count": sum(1 for r in relay_parsed if r["action_state"] == "drop_candidate"),
            },
            "meta": {
                "total_signals": len(ordinary_parsed) + len(relay_parsed),
                "hint": "pending_open_check = 明日人工检查T+1数据后再决定是否买入",
            },
        }

    def fetch_positions(self, trade_date: Optional[str] = None) -> Dict[str, Any]:
        """Return open and recently closed positions from strategy_trade_journal."""
        query = """
            SELECT
                trade_id,
                signal_date,
            ...
            WHERE (:td IS NULL OR COALESCE(sell_date, buy_date, signal_date) >= :td)
            ORDER BY COALESCE(buy_date, signal_date) DESC, model, stock_code
            LIMIT 200
        """
        with self.engine.connect() as conn:
            rows = [dict(row) for row in conn.execute(text(query), {"td": trade_date}).mappings().all()]

        open_pos = [r for r in rows if r.get("trade_status") != "CLOSED"]
        closed_pos = [r for r in rows if r.get("trade_status") == "CLOSED"]

        def serialize_notes(val: Any) -> str:
            if val is None:
                return ""
            if isinstance(val, str):
                return val
        ...
        for r in rows:
            r["manual_notes"] = serialize_notes(r.get("manual_notes"))

        total_pnl = sum(float(r["pnl"]) for r in closed_pos if r.get("pnl") is not None)
        total_pnl_pct = [float(r["pnl_pct"]) for r in closed_pos if r.get("pnl_pct") is not None]
        avg_pnl_pct = sum(total_pnl_pct) / len(total_pnl_pct) if total_pnl_pct else 0.0
        wins = sum(1 for p in total_pnl_pct if p > 0)
        losses = sum(1 for p in total_pnl_pct if p < 0)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/plan":
            ...
            if not trade_date:
                _json(self, {"error": "no trade_date and no data in stock_daily"}, status=400)
                return
            _json(self, self.app.fetch_plan(trade_date))
            return
        if path == "/api/positions":
            trade_date = (q.get("trade_date") or [""])[0]
            trade_date = str(trade_date).strip() or None
            _json(self, self.app.fetch_positions(trade_date=trade_date))
            return
